<template>
  <div class="app-container">
    <el-card class="operate-container" shadow="never">
      ...
      <el-button
        class="btn-add"
        @click="handleAddProductAttrCate()"
        size="mini">
        添加
      </el-button>
    </el-card>
      ...
      <el-table ref="productAttrCateTable"
        ...
        <el-table-column label="设置" width="200" align="center">
          <template slot-scope="scope">
            <el-button
              size="mini"
              @click="handleShowAttr(scope.$index, scope.row)">属性列表
            </el-button>
            <el-button
              size="mini"
              @click="handleShowParam(scope.$index, scope.row)">参数列表
            </el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleAddProductAttrCate() {
      this.dialogTitle = '添加商品属性分类'
      this.dialogVisible = true
      this.productAttrCate = { name: '' }
    },
    handleUpdate(index, row) {
      this.dialogTitle = '修改商品属性分类'
      this.dialogVisible = true
      this.productAttrCate = { name: row.name, id: row.id }
    },
      ...
      this.$refs['productAttrCatForm'].validate((valid) => {
        if (valid) {
          if (this.dialogTitle === '添加商品属性分类') {
            ...
            updateProductAttrCate(this.productAttrCate.id, this.productAttrCate).then(response => {
              this.$message({
                message: '修改成功',
                type: 'success',
                duration: 1000
